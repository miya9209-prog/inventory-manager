...
class Cafe24Client:
    ...
    def exchange_code(self, code):
        ...
        r = requests.post(url, headers=self._basic_auth_headers(), data=data, timeout=30)
    ...
    def fetch_options(self, product_no):
        url = f"{self.base}/admin/products/{product_no}/options"
        return self._get(url)

    def fetch_variants(self, product_no):
        url = f"{self.base}/admin/products/{product_no}/variants"
        return self._get(url)
    ...
    def get_option_inventory_rows(self, product_no):
        ...
        try:
            ...
            option_items = self._extract_list(data)

            for obj in option_items:
        ...
        if not rows or sum(r["cafe24_stock"] for r in rows) == 0:
            try:
                ...
                variant_items = self._extract_list(data)

                variant_rows = []
                for obj in variant_items:
                    qty = self._pick_qty(obj)
                    variant_rows.append({
                        "product_no": str(product_no),
                        "option_name": self._option_name(obj),
